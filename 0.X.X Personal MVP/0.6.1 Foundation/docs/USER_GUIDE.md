...
**Summon Command (v0.6.1):**
- `/summon <message>` — Talk directly to Butler (bypasses fast mode)
- `/summon status` — Detailed system status
- `/summon what tasks` — Query top tasks
- `/summon what projects` — Query active projects
- `/summon correct thought <id>` — Correct a specific thought

**Maintenance Commands (v0.6.1):**
- `maintenance models` — Discover and benchmark available LLM models
- `maintenance scan` — Run full system maintenance scan
- `maintenance insights` — View pending maintenance insights
...
### /summon Command (v0.6.1)
The `/summon` command bypasses fast-mode interpretation and talks directly to the Butler.

**Use cases:**
- Override a fast-mode decision that was wrong
- Get slow-mode analysis immediately instead of waiting
- Query detailed system state
- Correct a previous thought or task

**Examples:**
```
/summon status              # Detailed system overview
/summon what tasks          # List top priority tasks
/summon correct thought 5   # Correct thought #5
/summon help me organize    # General Butler assistance
```

The Butler will respond within 30 seconds or queue the request for later.

### Maintenance Protocol (v0.6.1)
Periodic system self-improvement through model discovery and health checks.

**Commands:**
```
maintenance models    # Discover and benchmark all Ollama models
...
**What it checks:**
- Available LLM models and their performance
- Slow work queue health (backlogs, failures)
- Project suggestion staleness
- Butler contact budget usage

**Maintenance reports** include actionable recommendations numbered for quick response.
...
| `/summon <msg>` | Talk directly to Butler (v0.6.1) |
| `maintenance models` | Discover/benchmark models (v0.6.1) |
| `maintenance scan` | Run maintenance scan (v0.6.1) |
| `maintenance insights` | View pending insights (v0.6.1) |
| `maintenance report` | Preview maintenance report (v0.6.1) |
...
- **v0.6.1** — Foundation Layer
  ...
  - **`/summon` command**: Bypass fast mode, talk directly to Butler with 30s timeout
  - **Model registry**: Discover and benchmark Ollama models; abstract backend for future expansion
  - **Maintenance protocol**: System self-improvement with insights, reports, and model health checks
  - New commands: `/summon`, `maintenance models/scan/insights/report`
  - New tables: `execution_logs`, `model_registry`, `maintenance_insights`
  - `summon_mode` flag on thoughts for correction tracking
  - 398 passing tests (44 new)
