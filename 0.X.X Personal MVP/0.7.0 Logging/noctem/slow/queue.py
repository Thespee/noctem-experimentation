...
class WorkType(Enum):
    ...
    LOG_REVIEW = "log_review"  # v0.7.0: Self-improvement engine
...
class SlowWorkQueue:
    ...
    @staticmethod
    def queue_log_review() -> int:
        """Queue a log review task (v0.7.0)."""
        # Use target_id=0 for system-level tasks like log review
        # Check if log review already queued
        with get_db() as conn:
            existing = conn.execute("""
                SELECT id FROM slow_work_queue
                WHERE work_type = ? AND status IN ('pending', 'processing')
            """, (WorkType.LOG_REVIEW.value,)).fetchone()
            
            if existing:
                return existing["id"]
        
        return SlowWorkQueue.add_item(WorkType.LOG_REVIEW.value, target_id=0)
