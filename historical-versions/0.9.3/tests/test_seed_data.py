...
class TestSeedDataLoading:
    ...
    def test_load_task_with_status_preserved(self):
        """Should preserve explicit task status from seed data."""
        from noctem.seed.loader import load_seed_data, ConflictAction
        from noctem.services import task_service

        data = {
            "tasks": [{"name": "Seed Done Status Task", "status": "done"}]
        }
        ...
        task = task_service.get_task_by_name("Seed Done Status Task")
...
class TestSeedDataExport:
    ...
    def test_export_task_includes_status_and_due_time(self):
        """Export should include task status and due_time metadata."""
        from datetime import time
        from noctem.seed.loader import export_seed_data
        from noctem.services import task_service

        marker = "Seed Export Status Time Task"
        task = task_service.create_task(marker, due_time=time(9, 0))
        task_service.complete_task(task.id)

        data = export_seed_data(include_tasks=True, include_done_tasks=True)
        exported = next((t for t in data["tasks"] if t["name"] == marker), None)
        assert exported is not None
        assert exported.get("status") == "done"
        assert exported.get("due_time") == "09:00:00"
