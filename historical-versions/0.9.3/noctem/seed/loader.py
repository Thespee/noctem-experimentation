...
def _mcp_create_task(
    *,
    ...
    result = get_mcp_server().call_tool(
        ...
        context={"source": "seed.loader"},
    )
    if not result.get("ok"):
        message = (result.get("error") or {}).get("message") or "unknown MCP create error"
        raise RuntimeError(message)
    task_payload = result["result"].get("task")
    if not isinstance(task_payload, dict) or task_payload.get("id") is None:
        raise RuntimeError("Invalid MCP create payload")
    return int(task_payload["id"])


def _mcp_update_task(task_id: int, updates: dict[str, Any]) -> None:
    args = {"task_id": int(task_id), **updates}
    result = get_mcp_server().call_tool(
        "tasks.update_fields",
        args,
        context={"source": "seed.loader"},
    ...
    if not result.get("ok"):
        message = (result.get("error") or {}).get("message") or "unknown MCP update error"
        raise RuntimeError(message)
    if result["result"].get("verified") is False:
        raise RuntimeError("MCP update verification failed")


def _get_existing_task_exact_name(name: str):
    with get_db() as conn:
        row = conn.execute(
            "SELECT id FROM tasks WHERE LOWER(name) = LOWER(?) ORDER BY created_at DESC, id DESC LIMIT 1",
            (name,),
        ).fetchone()
    if not row:
        return None
    return task_service.get_task(int(row["id"]))
...
def load_seed_data(
    ...
    for task_data in data.get("tasks", []):
        try:
            name = task_data["name"]
            existing = _get_existing_task_exact_name(name)
            # Preserve duplicate names within the same import batch by only
            # treating pre-import tasks as conflicts.
            if existing and existing.id not in preexisting_task_ids:
                existing = None
            ...
            if "due_date" in task_data:
                due_date_iso = date.fromisoformat(task_data["due_date"]).isoformat()
            due_time_iso = None
            if "due_time" in task_data and task_data["due_time"]:
                due_time_iso = str(task_data["due_time"]).strip()
            recurrence_rule = task_data.get("recurrence_rule") or task_data.get("recurrence")
            ...
            if existing:
                ...
                if action == ConflictAction.SKIP:
                    ...
                    if "due_date" in task_data:
                        update_fields["due_date"] = due_date_iso
                    if "due_time" in task_data:
                        update_fields["due_time"] = due_time_iso
                    if "tags" in task_data:
                        update_fields["tags"] = task_data.get("tags")
                    if "recurrence_rule" in task_data or "recurrence" in task_data:
                        update_fields["recurrence_rule"] = recurrence_rule
                    if "status" in task_data and task_data.get("status"):
                        update_fields["status"] = str(task_data["status"])
                    _mcp_update_task(existing.id, update_fields)
            ...
            # Create new task
            created_task_id = _mcp_create_task(
                name=name,
                project_id=project_id,
            ...
            if status and status != "not_started":
                _mcp_update_task(created_task_id, {"status": status})
    ...
    if include_tasks:
        ...
        for task in tasks:
            ...
            if task.recurrence_rule:
                ...
                task_data["recurrence_rule"] = task.recurrence_rule
