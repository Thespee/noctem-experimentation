...
def _idempotent_result(operation: str, idempotency_key: str | None) -> dict[str, Any] | None:
    if not idempotency_key:
        return None
    return _COMMIT_RESULT_STORE.get((operation, idempotency_key))
...
def _tool_audit_list_events(arguments: dict[str, Any], _: MCPRequestContext) -> dict[str, Any]:
    ...
    if operation:
        events = [event for event in events if str(event.get("operation") or "").strip().lower() == operation]
    if limit > 0:
        events = events[:limit]
...
def _tool_audit_get_event(arguments: dict[str, Any], _: MCPRequestContext) -> dict[str, Any]:
    ...
    event = next((item for item in _AUDIT_EVENTS if item.get("event_id") == event_id), None)
    return {"event": event}
...
def _tool_undo_preview(arguments: dict[str, Any], _: MCPRequestContext) -> dict[str, Any]:
    ...
    if event_id is not None:
        event = next((item for item in _AUDIT_EVENTS if item.get("event_id") == str(event_id)), None)
    else:
        for candidate in reversed(_AUDIT_EVENTS):
