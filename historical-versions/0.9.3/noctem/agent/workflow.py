...
def _interrupt(
    ...
    *,
    ...
    _update_workflow(workflow_id, status="interrupted", current_node="pending_approval")
    log_agent_action(
        workflow_id,
        "interrupted",
        input_data=context,
        output_data={"interrupt_id": interrupt_id, "question": question},
...
def resume_workflow(workflow_id: int, resolution: str) -> dict | None:
    ...
    interrupt = pending[0]
    resolve_interrupt(interrupt["id"], decision)
    _update_workflow(workflow_id, status="active", current_node="resumed")
    log_agent_action(
        workflow_id,
        "resumed",
        input_data={"interrupt_id": interrupt["id"], "resolution": decision},
        output_data={},
    ...
    if intent == IntentType.ADD_TASK.value:
        combined = f"{original_text} {decision}".strip()
        return _handle_add_task(workflow_id, combined, allow_interrupt=False)
    if intent == IntentType.COMPLETE_TASK.value:
        return _handle_complete_task(workflow_id, decision)
    if intent == IntentType.SKIP_TASK.value:
        return _handle_skip_task(workflow_id, decision)
    if intent == IntentType.DELETE_TASK.value:
        if stage == "confirm_delete":
            ...
            if _is_negative(decision):
                ...
                return _complete(workflow_id, cancelled_response, output={"deleted_task_id": None})
            return _interrupt(
                workflow_id,
                interrupt_type="approve",
                question="Please reply with 'yes' or 'no' to confirm deletion.",
                options=["yes", "no"],
                context=context,
                reasoning="Deletion approval requires explicit yes/no response",
            )
        return _handle_delete_task(workflow_id, decision)
    if intent == IntentType.BULK_ADD.value:
        return _handle_bulk_add(workflow_id, decision)
    if intent == IntentType.BULK_EDIT.value:
        if stage == "confirm_bulk_edit":
            if _is_affirmative(decision):
                replay_text = original_text or context.get("original_text") or ""
                return _handle_bulk_edit(
                    workflow_id,
                    replay_text,
            ...
            if _is_negative(decision):
                ...
                return _complete(workflow_id, cancelled_response, output={"updated_count": 0, "updated_task_ids": []})
            return _interrupt(
                workflow_id,
                interrupt_type="approve",
                question="Please reply with 'yes' or 'no' to approve this bulk edit.",
                options=["yes", "no"],
                context=context,
                reasoning="Bulk edit approval requires explicit yes/no response",
            )
        combined = f"{original_text} {decision}".strip() if original_text else decision
        return _handle_bulk_edit(workflow_id, combined, allow_interrupt=False)
    if intent == IntentType.QUERY.value:
        return _handle_query(workflow_id, decision)

    return _complete(workflow_id, "✓ Workflow resumed.")


def list_pending_interrupts() -> list[dict]:
    """List all unresolved agent interrupts."""
    return get_pending_interrupts()
