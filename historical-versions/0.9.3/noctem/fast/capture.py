...
def _handle_actionable(
    ...
    create_result = get_mcp_server().call_tool(
        "tasks.create",
        {
            "name": parsed.name,
        ...
        context={"source": "fast.capture", "thread_id": f"thought-{thought.id}"},
    )
    if not create_result.get("ok"):
        ...
        return CaptureResult(
            ...
            kind=ThoughtKind.AMBIGUOUS,
            confidence=classification.confidence,
            response="I couldn't save that task right now. I'll ask about it later.",
        )
    created_payload = create_result["result"].get("task")
    if not isinstance(created_payload, dict) or created_payload.get("id") is None:
        update_thought(thought.id, kind="ambiguous")
        return CaptureResult(
            thought_id=thought.id,
            kind=ThoughtKind.AMBIGUOUS,
            confidence=classification.confidence,
            response="I couldn't verify that saved task. I'll ask about it later.",
        )
    task = task_service.get_task(int(created_payload["id"]))
    if task is None:
        update_thought(thought.id, kind="ambiguous")
        return CaptureResult(
            thought_id=thought.id,
