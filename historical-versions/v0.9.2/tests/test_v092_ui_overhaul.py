...
class TestWhisperGracefulImport:
    """Test that whisper module loads without crashing even if faster-whisper is missing."""

    def test_whisper_module_imports(self):
        """Importing whisper module should never raise."""
        from noctem.slow import whisper
        assert hasattr(whisper, "WhisperService")
        assert hasattr(whisper, "_FASTER_WHISPER_AVAILABLE")
    ...
    def test_is_ready_reflects_availability(self):
        from noctem.slow.whisper import WhisperService, _FASTER_WHISPER_AVAILABLE
        svc = WhisperService()
