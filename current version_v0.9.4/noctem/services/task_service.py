...
def create_task(
    ...
    if importance is None:
        importance = 0.5
    due_date_db = due_date.isoformat() if isinstance(due_date, date) else due_date
    due_time_db = due_time.isoformat() if isinstance(due_time, time) else due_time
    ...
    log_action(
        ...
        "task",
        task_id,
        {"name": name, "due_date": str(due_date_db) if due_date_db else None, "importance": importance},
    )
...
def update_task(
    ...
    project_id: Optional[int] | object = _UNSET,
    status: Optional[str] | object = _UNSET,
    due_date: Optional[date] | object = _UNSET,
    due_time: Optional[time] | object = _UNSET,
    importance: Optional[float] | object = _UNSET,
    tags: Optional[list[str]] | object = _UNSET,
    recurrence_rule: Optional[str] | object = _UNSET,
) -> Optional[Task]:
    """Update a task."""
    updates = []
    params = []
    ...
    if due_date is not _UNSET:
        ...
        params.append(due_date.isoformat() if isinstance(due_date, date) else due_date)
    if due_time is not _UNSET:
        updates.append("due_time = ?")
        params.append(due_time.isoformat() if isinstance(due_time, time) else due_time)
...
def get_tasks_with_suggestions(limit: int = 5) -> list[Task]:
    """Get tasks that have AI suggestions (v0.6.0)."""
    with get_db() as conn:
        rows = conn.execute(
            """
            SELECT * FROM tasks 
            WHERE computer_help_suggestion IS NOT NULL 
