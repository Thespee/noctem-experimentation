...
def record_message(
    ...
    if session_id is None:
        session_id = resolve_thread_id(source=source)
...
def record_thinking(
    ...
    Args:
        ...
        source: Which component generated this ('agent', 'slow', 'sync', etc.)
...
def get_thread_context(
    ...
    """
    Get recent messages for a durable thread ID (oldest first).
    """
    if not thread_id:
        return []
...
def resolve_thread_id(source: str, thread_id: Optional[str] = None) -> str:
    """
    Resolve the durable chat thread ID.

    Priority:
    1) Explicit caller-provided thread_id
    2) Unified shared thread configured for chat sources
    3) Legacy source-scoped rolling session
    """
    provided = (thread_id or "").strip()
    if provided:
        return provided

    if source in {"web", "cli", "telegram"} and Config.chat_unified_continuity():
        shared = Config.chat_default_thread_id()
        if shared:
            return shared

    return _get_or_create_session(source)
