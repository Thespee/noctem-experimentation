"""Instagram fingerprint scanner for Cor Unum artists."""
...
_DISCOVERY_COLUMN_MIGRATIONS = (
    ("instagram_last_discovery_attempt_at", "TIMESTAMP"),
    ("instagram_discovery_error", "TEXT"),
)
...
def _has_discovery_columns(conn) -> bool:
    ...
    return {"instagram_last_discovery_attempt_at", "instagram_discovery_error"}.issubset(cols)
...
def _profile_signals(url: str) -> tuple[bool, bool]:
    ...
    is_canadian = is_local or "canada" in text or "bc" in text
    return is_local, is_canadian


def check_artist_instagram_fingerprint(artist_id: int, force: bool = False) -> dict:
    attempt_at = datetime.utcnow().isoformat()
    with get_db() as conn:
        _ensure_discovery_columns(conn)
        has_discovery_cols = _has_discovery_columns(conn)
        row = conn.execute(
            """SELECT id, name, alias_of, instagram_url, is_canadian, instagram_checked_at
        ...
        if row["alias_of"]:
            return check_artist_instagram_fingerprint(row["alias_of"], force=force)
        instagram_url = (row["instagram_url"] or "").strip()
        discovered = False
        if not instagram_url:
            try:
                discovered_match = discover_best_profile_url(row["name"], "instagram")
            except DiscoveryProviderUnavailable as exc:
                if has_discovery_cols:
                    conn.execute(
                        """UPDATE cu_artists
                           SET instagram_last_discovery_attempt_at = ?, instagram_discovery_error = ?
                           WHERE id = ?""",
                        (attempt_at, str(exc)[:500], artist_id),
                    )
                return {"error": str(exc)}
            if discovered_match and discovered_match.get("candidate_url"):
                instagram_url = discovered_match["candidate_url"].strip()
                if has_discovery_cols:
                    conn.execute(
                        """UPDATE cu_artists
                           SET instagram_url = ?, instagram_last_discovery_attempt_at = ?, instagram_discovery_error = NULL
                           WHERE id = ?""",
                        (instagram_url, attempt_at, artist_id),
                    )
                else:
                    conn.execute(
                        "UPDATE cu_artists SET instagram_url = ? WHERE id = ?",
                        (instagram_url, artist_id),
                    )
                discovered = True
            else:
                if has_discovery_cols:
                ...
                return {"error": "No Instagram URL found for this artist"}
        checked_at = row["instagram_checked_at"]
    ...
    is_local, is_canadian = _profile_signals(instagram_url)
    with get_db() as conn:
        _ensure_discovery_columns(conn)
        has_discovery_cols = _has_discovery_columns(conn)
            ...
            conn.execute(
                """UPDATE cu_artists
                   SET is_canadian = ?,
                       ...
                       instagram_last_discovery_attempt_at = ?,
                       instagram_discovery_error = NULL
                   WHERE id = ?""",
                (1 if is_canadian else 0, datetime.utcnow().isoformat(), attempt_at, artist_id),
            )
        else:
            conn.execute(
                "UPDATE cu_artists SET is_canadian = ?, instagram_checked_at = ? WHERE id = ?",
...
def check_instagram_fingerprints(limit: int = 150, recheck_all: bool = False) -> dict:
        ...
        if recheck_all:
            ...
            rows = conn.execute(
                """SELECT id, name, instagram_url
                   FROM cu_artists
                   WHERE alias_of IS NULL
                     AND instagram_checked_at IS NULL
                     AND (
                       instagram_last_discovery_attempt_at IS NULL
                       OR instagram_last_discovery_attempt_at < ?
                     )
                   ORDER BY last_seen DESC NULLS LAST
                   LIMIT ?""",
                (retry_cutoff, limit),
        ...
        for row in rows:
            ...
            if not instagram_url:
                try:
                    ...
                    logger.warning("Aborting instagram fingerprint run: %s", exc)
                    break
                if discovered_match and discovered_match.get("candidate_url"):
                    instagram_url = discovered_match["candidate_url"].strip()
                    if has_discovery_cols:
                        conn.execute(
                            """UPDATE cu_artists
                               SET instagram_url = ?, instagram_last_discovery_attempt_at = ?, instagram_discovery_error = NULL
                               WHERE id = ?""",
                            (instagram_url, attempt_at, row["id"]),
                        )
                    else:
                        conn.execute(
                            "UPDATE cu_artists SET instagram_url = ? WHERE id = ?",
                            (instagram_url, row["id"]),
                        )
