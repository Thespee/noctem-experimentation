...
def _discover_instagram_url(artist_name: str) -> dict:
    ...
    for variant in variants:
        for attempt in range(1, DISCOVERY_PROVIDER_RETRY_ATTEMPTS + 1):
            try:
                discovered_match = discover_best_profile_url(variant, "instagram")
...
def check_artist_instagram_fingerprint(artist_id: int, force: bool = False) -> dict:
    ...
    with get_db() as conn:
        ...
        if not instagram_url:
            discovery_result = _discover_instagram_url(row["name"])
            discovery_status = str(discovery_result.get("status") or "")
            if discovery_status == "provider_unavailable":
                discovery_error = str(
                    discovery_result.get("error") or "Instagram discovery provider unavailable"
                )
            ...
            if discovery_status == "found" and discovery_result.get("candidate_url"):
                instagram_url = str(discovery_result.get("candidate_url") or "").strip()
                ...
                return {
                    "error": "No Instagram URL found for this artist",
...
def check_instagram_fingerprints(limit: int = 30, recheck_all: bool = False) -> dict:
    with get_db() as conn:
        _ensure_discovery_columns(conn)
